--[[
    GHAA SCRIPTER ROBLOX - Advanced Announcement System (SERVER SCRIPT - FIXED)
    Command: !a (message)
    Admin Only: ghawan2
...
-- Function to show error message to player
local function showErrorMessage(player, message)
...
-- Function to show cooldown message to player
local function showCooldownMessage(player, remainingTime)
...
local function handleAnnouncementRequest(player, message)
    ...
    if cooldowns[player.UserId] and (currentTime - cooldowns[player.UserId]) < ANNOUNCEMENT_COOLDOWN then
        ...
        showCooldownMessage(player, remainingTime)
        return
    end
    ...
    if not message or message == "" then
        showErrorMessage(player, "❌ Message cannot be empty!")
        return
    end
...
local function onPlayerAdded(player)
    player.Chatted:Connect(function(message)
        if string.sub(message:lower(), 1, #COMMAND_PREFIX + 1) == COMMAND_PREFIX .. " " then
            ...
            handleAnnouncementRequest(player, announcementMessage)
        end
    end)
end

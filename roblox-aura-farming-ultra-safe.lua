...
-- ========================================
-- ULTRA SAFE CHARACTER FUNCTIONS
-- ========================================
...
-- ========================================
-- ULTRA SAFE ANIMATION FUNCTIONS
-- ========================================

-- Play Animation Function (ULTRA SAFE VERSION)
function playAnimation(animationName)
    print("🎭 Attempting to play: " .. animationName)
    
    -- ULTRA SAFETY CHECK: Character must be ready
    ...
    -- Get current character and humanoid safely
    local currentChar = getCharacter()
    local currentHum = getHumanoid()
    
    if not currentChar or not currentHum then
        print("❌ Cannot get character or humanoid!")
        return false
    end
    ...
    -- Load animation ULTRA SAFELY
    local success, errorMsg = pcall(function()
        currentAnimationTrack = currentHum:LoadAnimation(animation)
...
-- Stop Animation Function (ULTRA SAFE VERSION)
...
-- ========================================
-- ULTRA SAFE EFFECT FUNCTIONS
-- ========================================

-- Play Sound Effect (ULTRA SAFE VERSION)
function playSoundEffect(animationName)
    ...
    -- Create sound ULTRA SAFELY
    local success, errorMsg = pcall(function()
        local sound = Instance.new("Sound")
        sound.SoundId = soundId
        sound.Volume = 0.5
        sound.Parent = currentChar
...
-- Create Visual Effects (ULTRA SAFE VERSION)
function createVisualEffects(animationName)
    ...
    -- Create aura effect ULTRA SAFELY
    local success, errorMsg = pcall(function()
        local aura = Instance.new("Part")
        aura.Name = "AuraEffect"
        aura.Size = Vector3.new(10, 10, 10)
        aura.Position = currentRoot.Position
        aura.Anchored = true
        aura.CanCollide = false
        aura.Transparency = 0.5
        aura.Parent = currentChar
        ...
        connection = RunService.Heartbeat:Connect(function()
            if aura.Parent and currentRoot then
                aura.CFrame = currentRoot.CFrame * CFrame.Angles(0, math.rad(1), 0)
            else
...
-- Stop Visual Effects (ULTRA SAFE VERSION)
function stopVisualEffects()
    ...
    local success, errorMsg = pcall(function()
        local aura = currentChar:FindFirstChild("AuraEffect")
        if aura then
            aura:Destroy()
        end
    end)
...
-- Keyboard Shortcuts (ULTRA SAFE VERSION)
